...
10.times do # Je définis 10 users aléatoirement etje lui affecte une des 10 villes créées
  User.create(first_name: Faker::TvShows::TwinPeaks.character, last_name: Faker::Name.last_name, description: Faker::TvShows::TwinPeaks.quote, email: Faker::Internet.email, age: Faker::Number.within(range: 15..50), city: City.all[rand(0..9)])
end

10.times do # Création de 20 gossips. Chaque gossip est attribué aléatoirement à un des users (un user peut ne pas avoir de gossip)
  Gossip.create(title: Faker::TvShows::TwinPeaks.location, content: Faker::TvShows::TwinPeaks.quote, user: User.all[rand(0..9)])
end

10.times do # Création de 10 tags aléatoires
  Tag.create(title: Faker::TvShows::TwinPeaks.location)
end
